...
for qq in range(num_modes):
    ...
    harris_coeffs_numaps[qq] = harris_coeffs_tmp  # arranged into 5 groups of 35 elements and in units of nm
...
plot_norm1 = TwoSlopeNorm(vcenter=0, vmin=-90,  vmax=90)
hcipy.imshow_field((nu_maps[0])*1e12, cmap='RdBu')  # nu_map is already in 1e-9 m
plt.tick_params(top=False, bottom=False, left=False, right=False, labelleft=False, labelbottom=False)
cbar = plt.colorbar()
cbar.ax.tick_params(labelsize=15)
cbar.set_label("$pm$", fontsize=15)

ax2 = plt.subplot2grid((2, 6), (0, 2), colspan=2)
plt.title("Segment Level 1mK bulk", fontsize=15)
plot_norm2 = TwoSlopeNorm(vcenter=0, vmin=-125,  vmax=50)
...
plot_norm3 = TwoSlopeNorm(vcenter=0, vmin=-75,  vmax=75)
...
plot_norm4 = TwoSlopeNorm(vcenter=0, vmin=-25,  vmax=25)
...
#plot_norm5 = TwoSlopeNorm(vcenter=0, vmin=-75,  vmax=75)
...
with open(os.path.join(data_dir, f'temp_static_tolerance_{c_target}.txt'), 'w') as file:
    file.write(f'\nAllowable temperature change (in mK) for DH target contrast:{c_target}')
    file.write(f'\nFaceplate silvered: {np.sqrt(np.mean(np.square(tel2.sm.actuators)))*1e3}')
    file.write(f'\nBulk: {np.sqrt(np.mean(np.square(tel3.sm.actuators)))*1e3}')
    file.write(f'\nGradient Radial: {np.sqrt(np.mean(np.square(tel4.sm.actuators)))*1e3}')
    file.write(f'\nGradient X Lateral: {np.sqrt(np.mean(np.square(tel5.sm.actuators)))*1e3}')
    file.write(f'\nGradient Z axial: {np.sqrt(np.mean(np.square(tel6.sm.actuators)))*1e3}')
