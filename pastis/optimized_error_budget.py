...
def build_mode_based_error_budget(design, run_choice, c_target=1e-10, error_budget='optimized', single_mode=None):
    ...
    if error_budget == 'optimized':
        ...
        # Build segment-space covariance matrix Ca
        Ca = np.diag(np.square(mus))
        hc.write_fits(Ca, os.path.join(workdir, 'results', 'Ca_{}.fits'.format(c_target)))

        # Calculate mode-space covariance matrix Cb
