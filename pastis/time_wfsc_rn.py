...
for tscale in np.logspace(TimeMinus,TimePlus,Ntimes):
    ...
    for wavescale in np.logspace(WaveScaleMinus,WaveScalePlus,Nwavescale):
        ...
        for StarMag in np.linspace(fluxMinus,fluxPlus,Nflux):
            ...
            tmp0 = req_closedloop_calc_batch(G_coron_LO,G_LOWFS,E0_coron,E0_LOWFS,dark_current+CIC/tscale,dark_current+CIC/tscale,tscale,flux*Starfactor,wavescale**2*QLO,niter,dh_mask,norm)
...
for tscale in np.logspace(TimeMinus,TimePlus,Ntimes):
    ...
    for wavescale in np.logspace(WaveScaleMinus,WaveScalePlus,Nwavescale):
        ...
        for StarMag in np.linspace(fluxMinus,fluxPlus,Nflux):
            Starfactor = 10**(-StarMag/2.5)
            tmp0 = req_closedloop_calc_recursive(G_coron_LO,G_LOWFS,E0_coron,E0_LOWFS,dark_current+CIC/tscale,dark_current+CIC/tscale,tscale,flux*Starfactor,wavescale**2*QLO,niter,dh_mask,norm)
            tmp1 = tmp0['averaged_hist']
            n_tmp1 = len(tmp1)
            res[pp,qq,rr] =  np.mean(tmp1[np.int(n_tmp1/2):n_tmp1]) - contrast_floor
            rr = rr + 1
...
res_line = np.reshape(res,[Ntimes*Nwavescale*Nflux])
text_files_name = overall_dir + '/LO_LOWFS_Recursive_dark_' + np.str(dark_current) + '_CIC_' + np.str(CIC) + '.csv'
np.savetxt(text_files_name, res_line, delimiter=",")
...
for tscale in np.logspace(TimeMinus,TimePlus,Ntimes):
    ...
    for wavescale in np.logspace(WaveScaleMinus,WaveScalePlus,Nwavescale):
        ...
        for StarMag in np.linspace(fluxMinus,fluxPlus,Nflux):
            Starfactor = 10**(-StarMag/2.5)
            tmp0 = req_closedloop_calc_batch(G_coron_LO,G_coron_LO_DH_nyquist,E0_coron,E0_coron_DH_nyquist,dark_current+CIC/tscale,dark_current+CIC/tscale,tscale,flux*Starfactor,wavescale**2*QLO,niter,dh_mask,norm)
            tmp1 = tmp0['averaged_hist']
            n_tmp1 = len(tmp1)
            res[pp,qq,rr] =  np.mean(tmp1[np.int(n_tmp1/2):n_tmp1]) - contrast_floor
            rr = rr + 1
...
res_line = np.reshape(res,[Ntimes*Nwavescale*Nflux])
text_files_name = overall_dir + '/LO_DH_Batch_dark_' + np.str(dark_current) + '_CIC_' + np.str(CIC) + '.csv'
np.savetxt(text_files_name, res_line, delimiter=",")
...
for tscale in np.logspace(TimeMinus,TimePlus,Ntimes):
    ...
    for wavescale in np.logspace(WaveScaleMinus,WaveScalePlus,Nwavescale):
        ...
        for StarMag in np.linspace(fluxMinus,fluxPlus,Nflux):
            Starfactor = 10**(-StarMag/2.5)
            tmp0 = req_closedloop_calc_recursive(G_coron_LO,G_coron_LO_DH_nyquist,E0_coron,E0_coron_DH_nyquist,dark_current+CIC/tscale,dark_current+CIC/tscale,tscale,flux*Starfactor,wavescale**2*QLO,niter,dh_mask,norm)
            tmp1 = tmp0['averaged_hist']
            n_tmp1 = len(tmp1)
            res[pp,qq,rr] =  np.mean(tmp1[np.int(n_tmp1/2):n_tmp1]) - contrast_floor
            rr = rr + 1
...
res_line = np.reshape(res,[Ntimes*Nwavescale*Nflux])
text_files_name = overall_dir + '/LO_DH_Recursive_dark_' + np.str(dark_current) + '_CIC_' + np.str(CIC) + '.csv'
np.savetxt(text_files_name, res_line, delimiter=",")
...
for tscale in np.logspace(TimeMinus,TimePlus,Ntimes):
    ...
    for wavescale in np.logspace(WaveScaleMinus,WaveScalePlus,Nwavescale):
        ...
        for StarMag in np.linspace(fluxMinus,fluxPlus,Nflux):
            Starfactor = 10**(-StarMag/2.5)
            tmp0 = req_closedloop_calc_batch(G_coron_MID,G_OBWFS,E0_coron,E0_OBWFS,dark_current+CIC/tscale,dark_current+CIC/tscale,tscale,flux*Starfactor,wavescale**2*QMID,niter,dh_mask,norm)
            tmp1 = tmp0['averaged_hist']
            n_tmp1 = len(tmp1)
            res[pp,qq,rr] =  np.mean(tmp1[np.int(n_tmp1/2):n_tmp1]) - contrast_floor
            rr = rr + 1
...
res_line = np.reshape(res,[Ntimes*Nwavescale*Nflux])
text_files_name = overall_dir + '/MID_OBWFS_Batch_dark_' + np.str(dark_current) + '_CIC_' + np.str(CIC) + '.csv'
np.savetxt(text_files_name, res_line, delimiter=",")
...
for tscale in np.logspace(TimeMinus,TimePlus,Ntimes):
    ...
    for wavescale in np.logspace(WaveScaleMinus,WaveScalePlus,Nwavescale):
        ...
        for StarMag in np.linspace(fluxMinus,fluxPlus,Nflux):
            Starfactor = 10**(-StarMag/2.5)
            tmp0 = req_closedloop_calc_recursive(G_coron_MID,G_OBWFS,E0_coron,E0_OBWFS,dark_current+CIC/tscale,dark_current+CIC/tscale,tscale,flux*Starfactor,wavescale**2*QMID,niter,dh_mask,norm)
            tmp1 = tmp0['averaged_hist']
            n_tmp1 = len(tmp1)
            res[pp,qq,rr] =  np.mean(tmp1[np.int(n_tmp1/2):n_tmp1]) - contrast_floor
            rr = rr + 1
...
res_line = np.reshape(res,[Ntimes*Nwavescale*Nflux])
text_files_name = overall_dir + '/MID_OBWFS_Recursive_dark_' + np.str(dark_current) + '_CIC_' + np.str(CIC) + '.csv'
np.savetxt(text_files_name, res_line, delimiter=",")
...
for tscale in np.logspace(TimeMinus,TimePlus,Ntimes):
    ...
    for wavescale in np.logspace(WaveScaleMinus,WaveScalePlus,Nwavescale):
        ...
        for StarMag in np.linspace(fluxMinus,fluxPlus,Nflux):
            Starfactor = 10**(-StarMag/2.5)
            tmp0 = req_closedloop_calc_batch(G_coron_MID,G_coron_MID_DH_nyquist,E0_coron,E0_coron_DH_nyquist,dark_current+CIC/tscale,dark_current+CIC/tscale,tscale,flux*Starfactor,wavescale**2*QMID,niter,dh_mask,norm)
            tmp1 = tmp0['averaged_hist']
            n_tmp1 = len(tmp1)
            res[pp,qq,rr] =  np.mean(tmp1[np.int(n_tmp1/2):n_tmp1]) - contrast_floor
            rr = rr + 1
...
res_line = np.reshape(res,[Ntimes*Nwavescale*Nflux])
text_files_name = overall_dir + '/MID_DH_Batch_dark_' + np.str(dark_current) + '_CIC_' + np.str(CIC) + '.csv'
np.savetxt(text_files_name, res_line, delimiter=",")
...
for tscale in np.logspace(TimeMinus,TimePlus,Ntimes):
    ...
    for wavescale in np.logspace(WaveScaleMinus,WaveScalePlus,Nwavescale):
        ...
        for StarMag in np.linspace(fluxMinus,fluxPlus,Nflux):
            Starfactor = 10**(-StarMag/2.5)
            tmp0 = req_closedloop_calc_recursive(G_coron_MID,G_coron_MID_DH_nyquist,E0_coron,E0_coron_DH_nyquist,dark_current+CIC/tscale,dark_current+CIC/tscale,tscale,flux*Starfactor,wavescale**2*QMID,niter,dh_mask,norm)
            tmp1 = tmp0['averaged_hist']
            n_tmp1 = len(tmp1)
            res[pp,qq,rr] =  np.mean(tmp1[np.int(n_tmp1/2):n_tmp1]) - contrast_floor
            rr = rr + 1
...
res_line = np.reshape(res,[Ntimes*Nwavescale*Nflux])
text_files_name = overall_dir + '/MID_DH_Recursive_dark_' + np.str(dark_current) + '_CIC_' + np.str(CIC) + '.csv'
np.savetxt(text_files_name, res_line, delimiter=",")
...
for tscale in np.logspace(TimeMinus,TimePlus,Ntimes):
    ...
    for wavescale in np.logspace(WaveScaleMinus,WaveScalePlus,Nwavescale):
        ...
        for StarMag in np.linspace(fluxMinus,fluxPlus,Nflux):
            Starfactor = 10**(-StarMag/2.5)
            tmp0 = req_closedloop_calc_batch(G_coron_HI,G_coron_HI_DH_nyquist,E0_coron,E0_coron_DH_nyquist,dark_current+CIC/tscale,dark_current+CIC/tscale,tscale,flux*Starfactor,wavescale**2*QHI,niter,dh_mask,norm)
            tmp1 = tmp0['averaged_hist']
            n_tmp1 = len(tmp1)
            res[pp,qq,rr] =  np.mean(tmp1[np.int(n_tmp1/2):n_tmp1]) - contrast_floor
            rr = rr + 1
...
res_line = np.reshape(res,[Ntimes*Nwavescale*Nflux])
text_files_name = overall_dir + '/HI_DH_Batch_dark_' + np.str(dark_current) + '_CIC_' + np.str(CIC) + '.csv'
np.savetxt(text_files_name, res_line, delimiter=",")
...
for tscale in np.logspace(TimeMinus,TimePlus,Ntimes):
    ...
    for wavescale in np.logspace(WaveScaleMinus,WaveScalePlus,Nwavescale):
        ...
        for StarMag in np.linspace(fluxMinus,fluxPlus,Nflux):
            Starfactor = 10**(-StarMag/2.5)
            tmp0 = req_closedloop_calc_recursive(G_coron_HI,G_coron_HI_DH_nyquist,E0_coron,E0_coron_DH_nyquist,dark_current+CIC/tscale,dark_current+CIC/tscale,tscale,flux*Starfactor,wavescale**2*QHI,niter,dh_mask,norm)
            tmp1 = tmp0['averaged_hist']
            n_tmp1 = len(tmp1)
            res[pp,qq,rr] =  np.mean(tmp1[np.int(n_tmp1/2):n_tmp1]) - contrast_floor
            rr = rr + 1
...
res_line = np.reshape(res,[Ntimes*Nwavescale*Nflux])
text_files_name = overall_dir + '/HI_DH_Recursive_dark_' + np.str(dark_current) + '_CIC_' + np.str(CIC) + '.csv'
np.savetxt(text_files_name, res_line, delimiter=",")
