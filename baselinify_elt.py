...
x=fits.open('x_center_luvoir.fits')[0].data
y=fits.open('y_center_luvoir.fits')[0].data
...
def matrix_calc(i):
    ...
    if i == 0: return_vec=np.array([0,i])
    if i !=0:
        for k in range(i):
            if np.abs(two_norm_reshaped[i]-two_norm_reshaped[k]) >= 4:
                return_vec=np.array([1,np.nan])
            if np.abs(two_norm_reshaped[i]-two_norm_reshaped[k]) < 4:
                if np.abs(np.cross(np.array([vec_list_x[i],vec_list_y[i]]),np.array([vec_list_x[k],vec_list_y[k]]))) >= 1000:
                    return_vec = np.array([1,np.nan])
                else:
                    return np.array([0,i])
    return return_vec
...
print('number of non_zero elements by (0,0) - elements: '+str(len(np.where((vec_list_reshaped[:,1] != 0) | (vec_list_reshaped[:,0] != 0))[0])))
